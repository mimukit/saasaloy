...
**Ground truth:** `docs/plans/0001-plan-saasaloy-build-spec-2026-07-21.md` — §2.7 (modules), §2.13 (agent context),
...
module owns, so patched files are **not** manifest-tracked as clean copies. `remove` reverses the six
kinds that edit a source or config file (`chained-route`, `wrangler-binding`,
`plugin-array`, `const-array`, `drizzle-column`, `nav-entry`) and drops the two `package.json` kinds with
