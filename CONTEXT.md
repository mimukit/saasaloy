...
Where a developer's own Postgres runs while `pnpm db:setup` prepares it: `docker` (the container the root `compose.yaml` defines), `server` (the host the existing `DATABASE_URL` names) or `neon` (a temporary neon.new database). A flag picks one on first run, the [state block](#state-block) remembers it, and `--reset` switches. One file per backend lives in `packages/db/scripts/backends/` ([ADR 0039](docs/adr/0039-adr-the-db-lifecycle-commands-belong-to-the-drivers-2026-09-20.md)).
_Avoid: provider. A [provider module](#provider-module) is a registry module a `<CAP>_PROVIDER` env var selects inside the Worker; a backend adds nothing to the registry, nothing at runtime reads it, and two developers on one repo may sit on different ones._

### State block
The run of comment lines `db:setup` writes at the top of `apps/api/.dev.vars`, naming the [backend](#backend-development-database), the database, when it was created and, on neon.new, when it expires. A comment is a line wrangler and `process.loadEnvFile` both skip, so the record rides along in the file that already holds `DATABASE_URL`. It is what makes `db:drop` safe: a `DATABASE_URL` the block does not vouch for is refused rather than dropped. The Postgres driver writes one; `database-d1` has none, because `apps/api/.wrangler/state` is already one database per worktree.
_Avoid: lock file, manifest. Nothing machine-owned at the repo root records it, and it is per checkout, never committed._
