...
### Provider module
A module supplying **one implementation of a capability's provider interface**: `email-cloudflare`, `email-console`, `sms-console`, `sms-khudebarta`, and the planned `email-resend` and `sms-twilio`. It carries a single file into the capability's `providers/` folder plus the patch that registers it, and it owns the descriptor surface that differs per provider — a binding, an npm dependency, a secret. `sms` shows the surface can be empty on one side and still worth the module: it has no Cloudflare-native provider at all, because Cloudflare has no SMS product. The authoring guide is `.agents/skills/create-provider/`. Several providers coexist in one project and `<CAP>_PROVIDER` picks between them at runtime, with no fallback in either direction. **A three-question test decides the shape**, not the word "stateless": a capability takes providers when the project owns neither the schema nor the migrations, and either a swap moves no data, or the alternate implementation *adds* one file where a [driver module](#driver-module) would *replace* files the core owns. The third question outranks the second, which is why `storage` takes providers although a swap copies every object by hand ([ADR 0033](docs/adr/0033-adr-transient-state-capabilities-take-providers-2026-09-08.md), amended by [ADR 0037](docs/adr/0037-adr-an-alternate-implementation-that-adds-one-file-is-a-provider-2026-09-08.md); the stateless/stateful wording of [ADR 0001](docs/adr/0001-adr-all-in-on-cloudflare-2026-07-22.md)'s 2026-08-04 amendment no longer decides). A provider promises only that a project can *start* on a different vendor. It promises no migration of data already written. The one-file rule covers the *runtime* surface only: a provider that needs a Workers binding may carry several descriptor patches, and it registers any [handler set](#handler-set) through `apps/api/src/worker.ts`, never through `infra`.
...
### Driver module
A module supplying **one of several mutually exclusive implementations of a capability whose records the project owns**: `database-d1` and `database-postgres` today. The deciding difference from a [provider module](#provider-module) is a file one: a driver **replaces** files the capability would otherwise own (`src/client.ts`, `drizzle.config.ts`, `tsconfig.json`), carries scaffolds of its own, and ships its own skill folder, where a provider **adds** one file to `providers/` and replaces none ([ADR 0037](docs/adr/0037-adr-an-alternate-implementation-that-adds-one-file-is-a-provider-2026-09-08.md)). Two drivers write the same paths, so they cannot coexist in one build, and that is what `conflictsWith` enforces. And unlike a provider, **exactly one** is ever installed, never zero and never two. Both halves are declared in descriptors and enforced by `saasaloy add`, never selected by a runtime env var: each driver names its siblings in `conflictsWith`, and the capability names its drivers in `requiresOneOf` so the core cannot land without one ([ADR 0026](docs/adr/0026-adr-database-driver-split-2026-08-28.md) and its 2026-08-31 amendment).
...
### Plan
A named tier of the product, declared in code in `packages/billing/src/plans.ts` with `definePlans`: an `id`, a display `name`, `features` (booleans), `limits` (numbers), an optional `trialDays`, and two price fields answering two kinds of vendor — `providerIds`, mapping a provider name to its price ids per interval, and `price`, mapping an interval to `{ amount, currency }` with the amount in the currency's minor unit. The list is the single source for both the [entitlement](#entitlement) answer and the provider's own price mapping — `billing-stripe` reads `providerIds.stripe.monthly` as the price id, `billing-sslcommerz` reads `price.monthly` because the gateway is handed a figure rather than a price id, and both read `trialDays` as the free-trial length. Exactly one plan is the default: it names no price through either field, and it is what a subject with no live [subscription](#subscription) resolves to. There is no plan table and no seed step. The landing page's `pricing-table` [block](#block) keeps its own copy in `content/landing.ts` and is kept in step by hand.
...
### Manual renewal
How a [subscription](#subscription) renews when the payment provider stores no instrument and has no recurring object of its own. `BillingProvider.renewal` is `"vendor"` unless a provider declares `"manual"`; `billing-sslcommerz` is the first that does. Each period is a fresh hosted checkout the subject pays: the daily `billing.renewal-due` job sets `past_due` on a row whose paid period has ended, sends the `renewal_due` notification once, and `POST /billing/renew` opens the next checkout for the plan already on the row. The existing lockout job then takes the plan away after `BILLING_LOCKOUT_DAYS` if nobody pays, exactly as it does after a refused card. The sweep filters on `billing_subscriptions.provider`, which the core writes from the event and no provider may set, so a project running both kinds of provider leaves each row to its own rules ([ADR 0039](docs/adr/0039-adr-billing-owns-a-provider-callback-surface-and-a-manual-renewal-mode-2026-09-21.md)).
_Avoid: invoicing, pay-as-you-go. Nothing is invoiced and nothing is metered — the price is the plan's, and only the schedule differs._

### Manual settlement
Who decides that money arrived. `BillingProvider.settlement` is `"vendor"` unless a provider declares `"manual"`, and `"manual"` means the payment method has no API at all — a personal bKash wallet, a bank transfer, cash — so the only witness is a person reading the receiving account's own statement. `billing` core answers that with a queue rather than an integration: it owns `billing_payment_submissions`, the subject's submit routes, the admin review routes and the **Payments** page, and a manual provider contributes only the instruction text, the field spec and the event an approval is worth. Nothing is verified automatically, and the capability promises nothing of the kind. An approval enqueues one `payment.succeeded` event, so the projection still has a single writer ([ADR 0040](docs/adr/0040-adr-a-payment-method-with-no-api-is-a-billing-provider-2026-09-21.md), [ADR 0034](docs/adr/adr-0034-billing-tables-are-a-projection-of-the-vendors-record-2026-09-08.md)).
_Avoid: offline payment, cash-on-delivery. Those are methods; settlement is the question of who witnesses one. Distinct from [manual renewal](#manual-renewal), which is about the schedule, not the witness — a provider may take either without the other._

### Payment submission
One row of `billing_payment_submissions`: a [billable subject](#billable-subject)'s claim that they paid, waiting for a person to agree. Opened at checkout with the plan, the interval and the **figure quoted at that moment**, filled in by the subject with a transaction reference and whatever else the provider's `submissionFields` asks for, and ended by an admin's approve or reject with a note. A subject has at most one `pending` submission at a time, and a partial unique index on `(provider, transaction_ref_normalized)` where `status <> 'rejected'` stops two subjects claiming one transaction — partial, so an admin who rejects a real payment by mistake does not burn that reference forever. The quoted amount is never recomputed from `plans.ts`: the subject paid what they were told to pay.
_Avoid: invoice, receipt, order. An invoice is a demand the vendor issues; this is a claim the subject makes, and it grants nothing until somebody says so._
...
### Object
A blob of bytes plus its metadata, held by whichever [provider module](#provider-module) `STORAGE_PROVIDER` selects and addressed by its [object key](#object-key). The `storage` capability moves objects and knows nothing else about them: no row, no owner, no tenant. The **record** of an object is a separate thing, a `storage_object` row the `file-uploads` feature owns and reads only through `@db/repositories/objects.ts` ([ADR 0037](docs/adr/0037-adr-an-alternate-implementation-that-adds-one-file-is-a-provider-2026-09-08.md)). The two can disagree, which is why `complete` verifies the object's real size against the row, and why deleting a record is a three-state soft delete rather than one write.
_Avoid: file (that is the user's word for the record), blob, asset._

### Object key
The full path an [object](#object) is stored under, always built by `buildKey({ tenantId, scope, id, filename })` into `t/<tenantId>/<scope>/<id>/<filename>`. A caller passes parts and never a path, so no route can write outside its tenant prefix and no filename can traverse; a key failing the pattern raises `StorageError` with code `invalid_key`. The per-object `id` in the middle is what lets a tenant re-upload a filename whose earlier record is soft-deleted.
_Avoid: path, filename, object name._

### Upload target
What `createUploadUrl` returns: `{ url, method, headers, expiresAt, direct }`, everything a client needs to PUT one object once. `direct: true` is a [presigned](#presign) vendor URL the browser writes to without touching the Worker. `direct: false` is a URL to the proxy route `packages/storage` ships, which streams the body through the binding. The caller does the same PUT either way, which is what lets `storage-memory` and an R2 bucket with no API token both work. `createDownloadUrl` returns the mirror of it. A target expires in 5 minutes for upload and 1 hour for download, and `expiresIn` is clamped at 24 hours.
_Avoid: signed URL (that is only the `direct: true` case), upload link._
