...
### Provider-owned job
A [job](#job) and its [schedule](#schedule) exported from a [provider module](#provider-module)'s own runtime file, rather than from the capability core. It exists for work the vendor forces outside the request: `billing-bkash-merchant` holds a gateway token on one, because bKash rate-limits the call that mints it to two an hour and an on-demand refresh would trip the limit. The module stays at one runtime file — the job registers through the capability's existing `jobs` and `schedules` [registration tables](#registration-table) in `packages/queue`, so a provider needs no scaffold and no route of its own. It reaches its credentials through a [provider env port](#provider-env-port), and it runs on every deploy of a project that installed the provider, so a job whose keys are unset returns rather than throwing ([ADR 0040](docs/adr/0040-adr-a-provider-may-depend-on-a-capability-and-own-a-scheduled-job-2026-09-21.md)).
_Avoid: background provider, daemon. It is an ordinary job; what is new is who ships it._

### Provider env port
A capability core's setter and reader pair carrying the Worker environment to a [provider-owned job](#provider-owned-job): `setBillingProviderEnv(env)` and `billingProviderEnv()` in `packages/billing/src/config.ts`, called once at module load from `apps/api/src/billing-store.ts`. A [job](#job) handler is called as `(payload, ctx)` and gets no `env`, and a core never reads `process.env`, so without the port a provider's scheduled work cannot reach its own secrets. The environment goes through **whole and opaque**, typed as the capability's own `BillingEnv`, so the core carries a provider's keys and names none of them — the same rule `createBilling(env)` already keeps on every request path. The reader throws before registration rather than answering an empty object, so a missing registration does not read as a missing secret. The sibling of `setBillingConfig`, which carries the one value the core itself needs ([ADR 0040](docs/adr/0040-adr-a-provider-may-depend-on-a-capability-and-own-a-scheduled-job-2026-09-21.md)).
_Avoid: config, secrets store. It registers no value and validates nothing; it hands one object across a boundary a handler signature cannot cross._
