...
- **Capability modules** — `api`, `database`, `validators`, `email`, `logger`, `auth`. Each scaffolds an app or package and establishes convention-based extension points (chained route registration, schema barrels, shared input schemas). `admin` is coming soon ([#13](https://github.com/mimukit/saasaloy/issues/13)).
