...
`e2e` is the capability that scaffolds `packages/e2e` (`@repo/e2e`): a Playwright suite that starts the installed apps on their pinned dev ports, drives them with headless Chromium, and stops every process it started. It `dependsOn` `api` and patches `e2e` and `e2e:install` into the root `package.json`. It takes neither providers nor drivers — it wraps no external service. Four flows ship, and each one declares the modules it needs as a Playwright tag; `playwright.config.ts` reads `installed` from `saasaloy.json` and never collects a flow whose modules are absent, rather than skipping it at runtime on a 404. Every spec belongs to this module, including the one that exercises `waitlist`, because a flow crosses modules and a spec shipped by a feature module would force the whole suite on every project that wanted that feature. See ADR 0040. The base template carries the other half: a root `vitest` and one example test, so `pnpm test` is green on a fresh project.
