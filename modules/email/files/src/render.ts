  ...
  return html`<!doctype html>
    <html lang="en">
      ...
      <body style="margin:0;padding:0;background-color:#f6f7f9;">
        <span
          style="display:none;font-size:0;line-height:0;max-height:0;opacity:0;overflow:hidden;"
          >${preheader}</span
        >
        <div
        ...
        >
          ...
          ${
            foot
              ? html`<hr
                    style="border:none;border-top:1px solid #e4e7eb;margin:32px 0 16px;"
                  />
                  <p
                    style="font-size:13px;line-height:1.5;color:#7b8794;margin:0;"
                  >
                    ${foot}
                  </p>`
              : ""
...
export function deriveText(source: string): string {
  return (
    source
      .replaceAll(/<!--[\s\S]*?-->/g, "")
      .replaceAll(/<(script|style|head)\b[\s\S]*?<\/\1\s*>/gi, "")
