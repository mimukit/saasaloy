...
| `billing-bkash-merchant` | `bkash-merchant` | a bKash merchant account: `BKASH_MERCHANT_APP_KEY`, `_APP_SECRET`, `_USERNAME`, `_PASSWORD`, optional `_MODE`, plus the `kv` capability and `KV_PROVIDER`. BDT only. See `modules/billing-bkash-merchant/skills/saasaloy-billing-bkash-merchant/` | **manually** — the subject pays each period | `handleCallback`, on `/billing/callback/bkash-merchant/*` |
