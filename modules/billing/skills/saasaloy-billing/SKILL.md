...
| `billing-bkash-personal` | `bkash-personal` | a personal bKash wallet and `BKASH_PERSONAL_NUMBER`. BDT only. No API exists, so every payment is approved by hand from the admin **Payments** page. See `modules/billing-bkash-personal/skills/saasaloy-billing-bkash-personal/` | **manually** — the subject pays each period | none — it settles manually instead |
...
A payment method with **no API at all** — a personal mobile-money wallet, a bank transfer, cash — declares `settlement: "manual"` instead, and implements three more members: `manualInstructions` (what the subject reads before they pay), `submissionFields` (what they type in after they pay, each field carrying its own `normalize`), and `approveSubmission` (the event an approved row is worth). `defineBilling` refuses a manual provider missing any of the three, at module load.

The queue behind those three is the core's, not the provider's. `billing` owns `billing_payment_submissions`, the subject's `GET`/`POST`/`DELETE /billing/submission` routes, the admin `/admin/billing/submissions` routes and the **Payments** page in `apps/admin`. Five rules hold there and a provider cannot change any of them: one pending submission per subject; the figure quoted at checkout is stored and never recomputed; a partial unique index on `(provider, transaction_ref_normalized)` where `status <> 'rejected'` refuses a duplicate claim while letting a rejected reference be submitted again; the review is a conditional update on `status = 'pending'` so two admins racing produce one grant and one 409; and an approval enqueues a `payment.succeeded` event rather than writing a subscription row. **Nothing is verified automatically** — see ADR 0040, and `billing-bkash-personal` as the worked example.
