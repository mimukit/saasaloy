...
// The customer link: which customer, at the payment provider, this user is. SQLite half,
// selected by `onlyWith: "database-d1"`; the Postgres twin is `user-billing.pg.ts`.
//
...
// `sqliteTable("user", …)` collides with `auth.sqlite.ts` in both. The applier has no patch
// kind that adds a property to a Drizzle table literal either — `plugin-array`,
// `const-array`, `chained-route`, `package-json-*` and `wrangler-binding` are the whole set
