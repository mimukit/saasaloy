...
plus the `range` on each `package-json-dependency` patch).
...
**A `package-json-dependency` patch range is the one pin you bump by hand.** It is scanned like any
other, so it appears in the report and it fails `deps:check`, but `deps:update` will not write it:
the write pass re-serializes a whole manifest, and that reflows a hand-authored descriptor's compact
arrays into a diff nobody wants to review. The report prints the exact edit — the file, the
`patches[]` index, and the version. Make it by hand and re-run. [#93](https://github.com/mimukit/saasaloy/issues/93)
tracks the formatting-preserving write that removes the step. Today `modules/waitlist`'s `hono` pin
is the only range this applies to; it must stay in step with `modules/api/files/package.json`'s
`hono`, or a generated project carries two copies of it across `apps/api` and `apps/web`.
