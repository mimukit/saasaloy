...
Success means three things hold on a fresh project: `saasaloy add teams` pulls in `auth` and `admin` and applies cleanly, running it twice changes nothing the second time, and a site admin can create an org, issue an invitation, and switch the active org from the admin shell.
...
| Invitation delivery | Copy-link. The admin UI shows the invitation URL for the inviter to copy. `teams` declares **no** dependency on the `email` capability, so `saasaloy add teams` never forces a provider choice. A later issue can add `sendInvitationEmail` behind the email capability without changing this module's dependency graph. |
...
| Removal (Q7) | In scope, best-effort. Give `plugin-array` and `const-array` inverses in the `INVERSES` table (`packages/cli/src/lib/patch/index.ts`), so `remove teams` restores the auth plugin array and the sidebar. Delete the schema file, and **warn explicitly** that the `organization`/`member`/`invitation` tables and their rows survive, naming `db:generate` as the step that would drop them. |
...
### Phase 1: the `const-array` patch kind
...
Drop `files/auth/plugins/organization.ts` → `@auth/plugins/organization.ts`, exporting `organizationPlugin()`. Leave `sendInvitationEmail` unset, with a comment explaining the copy-link decision and what to change once `email` is installed. Name `better-auth/plugins/organization/access` as the extension point for a custom role set.
...
- An invite-by-email form that renders the resulting invitation link for the inviter to copy, plus revoke
...
Write `modules/teams/skills/saasaloy-teams/SKILL.md` in the shape the other module skills use. It must state three things plainly: the screen is site-admin-only, it shows the caller's own memberships rather than every org, and there is no invitation-acceptance UI. Register the module wherever the registry index and the `wikikit` capability list live. Add the glossary entry separating the product word "team" from the plugin's off-by-default `teams` sub-feature.

### Phase 7: verify on a real project

In `.dev`, scaffold a project, run `saasaloy add teams` against a project with no `auth` and confirm the prerequisite resolves. Run it a second time and confirm a zero diff. Run `db:generate` and `db:migrate:local`. Then drive the flow: sign up, promote to admin, create an org, issue an invitation, accept it with a direct `POST /auth/organization/accept-invitation`, switch the active org. Run `saasaloy remove teams` and confirm both patches reverse and the data warning prints. Finish with the four-pass `pnpm lint` and `pnpm typecheck` on this repo and on the generated project.
...
None. The grill on 2026-08-31 closed all nine.
...
## Issue changes this plan implies

Issue #16 needs three edits before it is filed as work:

1. Remove the stale "Blocked by #13" line and the re-block comment. #13 is closed.
2. Correct the spec reference. The body cites `docs/plans/saasaloy-build-spec.md` §2.3 and §3.4; the real file is `docs/plans/plan-saasaloy-build-spec-2026-07-21.md`.
3. Rewrite the second acceptance criterion to "a site admin can create an org, issue an invitation, and switch active org", per Q6.
