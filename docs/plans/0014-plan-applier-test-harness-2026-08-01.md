...
| Decision | Resolution |
|----------|-----------|
| **Scope** | Tests plus `saasaloy doctor` only. Making `add` transactional is a **behavior** change with its own design question and lives in `0018-plan-transactional-add-2026-08-01.md`. This plan is purely additive — it may *expose* bugs, but it fixes none. |
...
- Cover the flows a unit test can't: `--dry-run` and `--diff` writing **nothing**, re-running `add`
  ...
  than clobbered.
- **Seam with `0017-plan-ship-the-cli-2026-08-01.md`:** that plan's Phase 5 release smoke test wants to
...
- **Matrix cadence: path-filtered PR run plus a nightly full run.** On a PR the matrix runs only when `modules/**` or the applier source changes; nightly runs everything. `0017-plan-ship-the-cli-2026-08-01.md` wires the workflow with this filter.
...
- **Making `add` transactional** — `0018-plan-transactional-add-2026-08-01.md`.
- **CI wiring** — `0017-plan-ship-the-cli-2026-08-01.md` owns the workflows; this plan owns the tests
  those workflows run.
