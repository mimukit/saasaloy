...
| **Major-version policy** | **Default-conservative.** The resolver's pre-checked default caps at the highest eligible version *within the current major*. A newer major is reported in its own `major-available` section and never crossed by default — only by a deliberate opt-in: ticking the picker's **Major** group, or `--allow-major` for a non-interactive run. Majors are where the template breaks, so each is blessed by hand. |
...
| **Output/DX stack** (Phase 7) | Reuse the CLI's `@clack/prompts` + `picocolors` (already lockfile deps via `packages/cli`) instead of raw `console.log`; add both as root `devDependencies`. Deliberately **drops the "zero-dep" constraint for this one maintainer script** — it's never shipped downstream and already assumes a pnpm install. Replicates `npm-check-updates` DX: semver-colored diffs, semver-grouped report with a dedicated Major-available section, spinner, and (revised on implementation) a **default** interactive `groupMultiselect` + `confirm` flow that merges `deps:check`/`deps:update` — `--yes` for non-interactive. Presentation only — resolver/write logic unchanged. |
...
- Sync the descriptor-shape doc: build-spec §3.3 (`docs/plans/0001-plan-saasaloy-build-spec-2026-07-21.md`) and `modules/README.md` where they show `["zod"]`.
...
> **Revised on implementation (2026-07-24).** After a first pass shipped the picker behind an opt-in `-i` flag, maintainer feedback merged **check + update into one interactive `deps:update`**: in a TTY it always shows the grouped report → a `groupMultiselect` picker → a `confirm`, then writes. The `-i`/`--interactive` flag is gone (interactive is the default); `--yes`/`-y` (and any non-TTY pipe) is the non-interactive escape. Majors moved from "flag-only" to their **own unchecked picker group** (checking one crosses the major; `--allow-major` remains for non-interactive runs), and the report gained a **dedicated Major-available section** so a bump like `astro 5 → 7` never hides inside a migration row. `deps:check` stays as the read-only CI gate. The bullets below describe the original design; the merged-flow specifics are the ones that shipped (see CONTRIBUTING "Updating dependencies" and ADR 0016).
...
- **Interactive default — opt-out vs opt-in.** → **Opt-out, and now the default flow.** Within-major bumps are pre-checked; the maintainer deselects exceptions. Superseded `-i`: the picker is no longer behind a flag — `deps:update` is interactive in a TTY and `--yes` opts out.
- **`major-available` coloring** → **red, in their own selectable group.** Majors render red and live in a dedicated unchecked `groupMultiselect` group (and a matching red report section); checking one is the deliberate cross-major opt-in, alongside `--allow-major` for non-interactive runs.
- **Check vs update as separate commands** → **merged.** `deps:update` subsumes the report + selection; `deps:check` remains only as the read-only CI gate (its `ELIFECYCLE`-on-drift exit is the gate working, not a bug — the daily command is `deps:update`).
