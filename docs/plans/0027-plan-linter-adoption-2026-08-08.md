...
| **`--deny-warnings` is a guard, not load-bearing today** | Measured against the composed config: all 523 enabled rules resolve to `error`; oxlint exits **0** on warnings by default, but this provider ships none. The flag stays as insurance against a future Ultracite release or a locally added rule shipping at `warn` — the silent-success failure this issue exists to delete. **Verified:** exit 0 without the flag, exit 1 with it, using a rule forced to `warn`. |
...
| **Generated projects ship the toolchain** | Same argument as [ADR 0022](../adr/0022-adr-design-layer-ships-in-the-base-2026-08-06.md) for the design layer: a non-functional layer earns its place in the base when every project wants it and an opt-in module would be a step everyone takes. |
...
- Update `docs/plans/0017-plan-ship-the-cli-2026-08-01.md` — the **Linter** row (line 37), the
...
- **`no-console`** with commented exemptions where console output *is* the feature:
  `packages/cli/src/index.ts` (terminal UX per [ADR 0009](../adr/0009-adr-cli-dx-clack-picocolors-2026-07-22.md)),
