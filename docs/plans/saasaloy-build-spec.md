...
- **Phase 0 — Base + agent scaffolding.** `saasaloy init` → Astro landing + `packages/ui` + `packages/config`, deploys green to Cloudflare. Also scaffolds the agent layer: canonical `.agents/` (base overview fragment + any base skills), `saasaloy.agent.json` targets, and `saasaloy sync` producing `AGENTS.md` + `CLAUDE.md` + `.claude/skills` links. Proves scaffolding + Turborepo + CF deploy + the canonical→tool-view pipeline.
- **Phase 1 — Applier + first proof.** Build the local applier (descriptor read, alias resolution, `dependsOn` topo-sort + confirmation, declarative + `magicast`/`jsonc-parser` patches, agent-fragment/skill drop + `sync`, `--dry-run`/`--diff`, `.saasaloy/manifest.json` hash tracking). Ship `api` + `database` capability modules and **`waitlist`** as the first end-to-end proof.
- **Phase 2 — Auth + hard proof + feature set.** `auth` + `admin` capability modules (httpOnly-cookie + subdomain flow). **`billing`** as the harder proof (Better Auth Stripe plugin + webhooks + pricing UI). Then `teams`, `email`, `admin` panel pages. Introduce the full update flow (migrations regen + smoke verify).
