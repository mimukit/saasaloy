...
- Add a note under §Templating in `docs/plans/0020-plan-email-capability-module-2026-08-04.md`: reversed by this plan, link both ways.
...
The decision is recorded in [ADR 0031](../adr/0036-adr-react-email-is-an-opt-in-render-engine-2026-09-03.md), which carries the same numbers.
...
End to end through `email-console`: `POST /mail` returned 200 with `{ subject, html, text }`, `html` starting `<!DOCTYPE html`, and the plaintext pass reading cleanly. The preview server started, listed `welcome`, and served the sample props. `@repo/api:typecheck` stays red on the two pre-existing `Bindings`/`LoggerEnv` errors that [ADR 0030](../adr/0030-adr-module-ui-ships-as-a-ui-package-block-2026-09-02.md) already records as present on main; every other workspace, `@repo/email-react` included, typechecks clean.
