...
| `billing-bkash-personal` | feature (provider) | a `billing` provider on a **personal** bKash wallet, which has no API of any kind. The subject sends money and submits the transaction ID; an admin checks it against the wallet's own statement and approves it from the **Payments** page. BDT only, renews manually, and nothing is verified automatically. Needs `BKASH_PERSONAL_NUMBER`, see [the reference](reference.md#the-other-providers-that-need-an-account) |
