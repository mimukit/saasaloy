...
| `sms-khudebarta` | feature (provider) | an `sms` provider on Khudebarta, a Bangladeshi bulk-SMS gateway, over plain HTTPS with no SDK and no binding. Sends to `+880` numbers only. Needs `KHUDEBARTA_API_KEY` and `KHUDEBARTA_SECRET_KEY`, see [the reference](reference.md#the-other-providers-that-need-an-account) |
...
| `billing` | capability | a `packages/billing` workspace with the provider contract, a `plans.ts` file, a billable-subject file, the `billing_subscriptions` projection and the provider registry, on `api` + `database` + `auth` + `admin` + `queue` + `email`. Zero runtime dependencies; `BILLING_PROVIDER` picks the provider at runtime with no default |
...
| `billing-bkash-merchant` | feature (provider) | a `billing` provider on bKash Tokenized Checkout, mode `0011`, over plain HTTPS with no SDK. BDT only, and it renews manually: bKash stores nothing a merchant may charge, so the subject pays for each period. Holds its gateway token on a scheduled job because bKash allows two token calls an hour, so it also needs the `kv` capability and `KV_PROVIDER`. Needs `BKASH_MERCHANT_APP_KEY`, `BKASH_MERCHANT_APP_SECRET`, `BKASH_MERCHANT_USERNAME` and `BKASH_MERCHANT_PASSWORD`, see [the reference](reference.md#the-other-providers-that-need-an-account) |
| `billing-sslcommerz` | feature (provider) | a `billing` provider on SSLCOMMERZ, Bangladesh's aggregator gateway, over plain HTTPS with no SDK. BDT only, and it renews manually: the gateway stores no card, so the subject pays for each period. Needs `SSLCOMMERZ_STORE_ID`, `SSLCOMMERZ_STORE_PASSWORD`, `SSLCOMMERZ_CALLBACK_URL` and `SSLCOMMERZ_RECEIPT_EMAIL`, see [the reference](reference.md#the-other-providers-that-need-an-account) |
| `entitlements` | feature | resolves what a subject's plan allows from `plans.ts` and the `billing_subscriptions` projection, and gates a route with `requireFeature` or `requireWithinLimit`, which answer 402, on `billing` |
