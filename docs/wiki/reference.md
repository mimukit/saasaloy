...
| `billing-bkash-merchant` | a [bKash](https://www.bkash.com/) merchant account; a sandbox one is issued at <https://developer.bka.sh>. `BKASH_MERCHANT_APP_KEY`, `BKASH_MERCHANT_APP_SECRET`, `BKASH_MERCHANT_USERNAME` and `BKASH_MERCHANT_PASSWORD` are all required once `BILLING_PROVIDER=bkash-merchant`; every bKash tutorial calls them `BKASH_APP_KEY` and so on, and the `MERCHANT` segment keeps them clear of a personal-account module's keys. `BKASH_MERCHANT_MODE` optionally selects the live gateway, and only the exact string `live` does so. The `kv` capability is installed with it and `KV_PROVIDER` must be set, because the gateway token is held between requests: bKash allows two token calls an hour, so a scheduled job mints it and staging must not share an app key with production. Plans must be priced in BDT through `price` in `plans.ts`, and the subject pays for each period: bKash stores nothing a merchant may charge. |
| `billing-sslcommerz` | an [SSLCOMMERZ](https://sslcommerz.com/) store; a sandbox one is issued in minutes at <https://developer.sslcommerz.com/registration/>. `SSLCOMMERZ_STORE_ID` and `SSLCOMMERZ_STORE_PASSWORD` are required once `BILLING_PROVIDER=sslcommerz`. `SSLCOMMERZ_CALLBACK_URL` is the https origin of your api Worker, under which the gateway's four callback URLs are built; register the IPN one in the merchant panel. `SSLCOMMERZ_RECEIPT_EMAIL` is a mailbox you own, because the gateway insists on a `cus_email` and a provider never learns the subject's own address. `SSLCOMMERZ_MODE` optionally selects the live gateway, and only the exact string `live` does so. Plans must be priced in BDT through `price` in `plans.ts`, and the subject pays for each period: the gateway stores no card. |
