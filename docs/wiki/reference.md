...
| `billing-bkash-personal` | a **personal** bKash wallet — not a merchant account, and there is no account to open and no API to call. `BKASH_PERSONAL_NUMBER` is the number subjects send money to and is required once `BILLING_PROVIDER=bkash-personal`; `BKASH_PERSONAL_ACCOUNT_LABEL` optionally names whose wallet it is. Plans must be priced in BDT through `price` in `plans.ts`. Every payment is approved by a person reading the wallet's own statement on the admin app's **Payments** page: nothing is verified automatically, and there is no sandbox to rehearse in. |
