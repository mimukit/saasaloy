...
| `sms` | nothing today. `sms-console` is the only provider there is, and it declares no `envVars`. `SMS_FROM` on the core is optional: a provider routing through a pool or a messaging service assigns the sender itself, and a provider that requires one rejects a send with `invalid_message` when neither `SMS_FROM` nor the message carries a `from`. |
