...
## Status
accepted — supersedes the draft's "auth in base". **Amended by [ADR 0022](adr-0022-design-layer-ships-in-the-base-2026-08-06.md):** the base now ships a Tailwind 4 + shadcn design layer, so the "no Tailwind/React" clause below no longer holds; the anti-rot thesis is narrowed to *functional* surfaces (no services, auth, database or network dependencies) and everything else here is unchanged.
