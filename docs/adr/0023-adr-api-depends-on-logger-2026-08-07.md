...
## Status
accepted — narrows [ADR 0018](0018-adr-database-depends-on-api-2026-07-24.md)'s "`api` remains a root capability with no `dependsOn`" for this edge. `api` is still the root of the *hosting* graph; it is no longer a graph root.
...
- **The dependency is `apps/api`'s alone, and it does not travel.** `@repo/logger` is declared in `apps/api/package.json`, so the code that can import it is API-owned code — `apps/api/src/**`, which is what `modules/api/files` scaffolds. `database`, `auth` and `waitlist` write into `packages/db`, `packages/auth` and `packages/web`: separate workspaces, and under pnpm's isolated `node_modules` a workspace resolves only what its own manifest declares. Logging from any of them means adding `@repo/logger` to that module's `files/package.json` first — a capability that logs owns its logger dependency, exactly as ADR 0020 has it.

## References
Plans: `docs/plans/0024-plan-logger-capability-module-2026-08-07.md`. Prior: [ADR 0005](0005-adr-two-tier-convention-based-modules-2026-07-22.md), [ADR 0013](0013-adr-module-dependency-ownership-and-scaffolds-files-split-2026-07-23.md), [ADR 0018](0018-adr-database-depends-on-api-2026-07-24.md), [ADR 0020](0020-adr-capability-owns-its-vendor-packages-2026-07-24.md). Issues: #66.
