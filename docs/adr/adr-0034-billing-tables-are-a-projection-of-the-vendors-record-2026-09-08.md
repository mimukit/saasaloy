...
- **A second provider is one file plus patches.** `billing-polar` needs one runtime file, two `plugin-array` patches (`billing.providers`, `auth.plugins`) and one `package-json-dependency` patch per npm package it pulls in. `billing-stripe` has three of those: `stripe`, `@better-auth/stripe`, and `better-auth` as a peer of the plugin. If a provider ever needs a second runtime file, the contract is wrong.
