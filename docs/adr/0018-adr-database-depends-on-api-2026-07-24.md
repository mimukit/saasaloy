...
The `api` plan and [ADR 0013](0013-adr-module-dependency-ownership-and-scaffolds-files-split-2026-07-23.md) framed `api` and `database` as independent **peers** ("neither needs the other"). But `database` must patch the D1 binding into `apps/api/wrangler.jsonc`, and a module cannot patch a file that isn't there. So `database` declares **`dependsOn: ["api"]`**: `saasaloy add database` resolves and installs `api` first, then patches its `wrangler.jsonc`. Settled while grilling issue #9 (the `database` capability module).

## Status
accepted — supersedes the "peer, neither needs the other" language in the `api` plan (`docs/plans/0005-plan-api-capability-module-2026-07-23.md`) and [ADR 0013](0013-adr-module-dependency-ownership-and-scaffolds-files-split-2026-07-23.md) for this edge only.
...
## References
Plans: `docs/plans/0008-plan-database-capability-module-2026-07-24.md`, `docs/plans/0005-plan-api-capability-module-2026-07-23.md`. Prior: [ADR 0005](0005-adr-two-tier-convention-based-modules-2026-07-22.md), [ADR 0013](0013-adr-module-dependency-ownership-and-scaffolds-files-split-2026-07-23.md). Issues: #9, #8.
</content>
