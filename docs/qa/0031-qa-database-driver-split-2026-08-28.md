...
```sh
grep -ln 'conflictsWith' docs/adr/*.md && grep -n 'ADR 0001\|single-provider\|stateful' docs/adr/0026-adr-database-driver-split-2026-08-28.md
```
...
- ✅ C11 → `docs/adr/0026-adr-database-driver-split-2026-08-28.md` exists and is the only ADR naming `conflictsWith`. It cites ADR 0001's 2026-08-04 amendment and argues the carve-out for a stateful capability. `docs/adr/` keeps no index file, so that clause of the check has nothing to satisfy.
