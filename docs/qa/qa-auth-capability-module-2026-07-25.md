...
> **Superseded in part on 2026-08-31 ([#98](https://github.com/mimukit/saasaloy/issues/98)).** `auth` no longer falls back to Better Auth's published development key when `BETTER_AUTH_SECRET` is unset. It throws at module scope, so the Worker fails on its first request instead of signing sessions with a well-known secret. The one escape hatch is a loopback `BETTER_AUTH_URL` (`http://localhost:4000` and friends), which is what keeps the keyless local loop the cases below rely on. `saasaloy add auth` now writes `apps/api/.dev.vars.example` with that line pre-filled. Every step below that expects a console **warning** about a missing secret expects the old behavior; on a non-loopback `BETTER_AUTH_URL` it is a thrown error now. The rest of the plan is unchanged.
