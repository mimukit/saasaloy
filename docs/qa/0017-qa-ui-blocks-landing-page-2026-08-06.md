...
- **Browser/OS matrix.** Only whatever browser the tester uses. Test on Tailwind 4's own supported target or newer — **Safari 16.4+, Chrome 111+, Firefox 128+** — which is the real floor here, above `oklch()`'s (Safari 15.4+ / Chrome 111+). Anything older is out of support, not a bug to file. `text-balance`/`text-pretty` degrade silently on older engines.
